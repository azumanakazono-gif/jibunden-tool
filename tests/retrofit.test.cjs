...
const sample={...defaults,generation:6000,exportKwh:4000,importKwh:4800,bill:160000,fixedBill:24000,buyRate:30,sellRate:8,capacity:10,reserve:0,efficiency:90,matching:100,nightShare:100,days:365,cost:100,years:15,degradation:0};
...
test('売電単価ゼロを保持し、充電・放電・料金上限を守る',()=>{
 const r=calculate({...sample,sellRate:0,importKwh:500,bill:10000,fixedBill:2000});
 assert.equal(r.first.lostSales,0);assert.equal(r.first.saving,8000);
 assert(r.first.discharge<=500);assert(r.first.charge<=sample.exportKwh);assert(r.first.billAfter===2000);
});
