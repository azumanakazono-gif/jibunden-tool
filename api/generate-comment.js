...
・出力は本文のみ。説明や見出し、鉤括弧は付けない`;
  ...
  // スタッフが電気料金プランや使用時間帯を個別に見直した場合、その内容がお客様に伝わるよう一言触れさせる
  const PLAN_REVIEW_RULE = (d.planChanged && d.postPlanLabel)
    ? `\n\n【プラン見直しへの追加ルール】\n・今回のご提案では、${d.beforePlanLabel ? `現在ご契約中の「${d.beforePlanLabel}」から、` : ''}導入後は「${d.postPlanLabel}」への切り替え・ご使用時間帯に合わせた見直しも合わせてご提案している\n・太陽光・蓄電池の導入だけでなく、お客様の暮らし方に合った料金プランまで丁寧に見直していることが伝わる一言を、本文中に自然に添えること\n・プラン名を書く場合は「${d.postPlanLabel}」という表記のまま使うこと（言い換え・誤字・架空のプラン名は不可）`
    : '';
