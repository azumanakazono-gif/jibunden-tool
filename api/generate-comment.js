...
・出力は本文のみ。説明や見出し、鉤括弧は付けない`;
  ...
  // 導入前後でプランが異なる場合のみ「料金プランの見直し」に言及させる。
  // プランが同じ場合は「プランの見直し」系の表現を一切禁止し、時間帯別比率等の調整時は生活リズムに合わせた設計として表現させる
  const planSwitched = !!(d.planSwitched && d.postPlanLabel);
  const PLAN_REVIEW_RULE = planSwitched
    ? `\n\n【プラン見直しへの追加ルール】\n・今回のご提案では、${d.beforePlanLabel ? `現在ご契約中の「${d.beforePlanLabel}」から、` : ''}導入後は「${d.postPlanLabel}」への切り替えも合わせてご提案している\n・「料金プランの見直しも含めた〜」という趣旨で、太陽光・蓄電池の導入だけでなく、お客様の暮らし方に合った料金プランまで丁寧に見直していることが伝わる一言を、本文中に自然に添えること\n・プラン名を書く場合は「${d.postPlanLabel}」という表記のまま使うこと（言い換え・誤字・架空のプラン名は不可）`
    : `\n\n【料金プランに関する禁止ルール】\n・今回は導入前後で電気料金プランを変更していないため、「プランの見直し」「プラン変更」「プランの切り替え」など料金プランを見直した・変更したと受け取れる表現は一切使わないこと${d.usageTuned
      ? `\n・その代わり、ご家族の生活リズムや時間帯別の電気の使い方に合わせて、発電した電気を無駄なく使うスマートなエネルギー設計をご提案していることが伝わる一言（例：「ご家族の生活リズムや時間帯別の使い方に合わせたスマートなエネルギー設計」）を、本文中に自然に添えること`
      : ''}`;
...
・ＡとＢを両方とも本文に書く必要はない。書く場合は1つの金額のみに絞ってもよい${HIGH_SELF_CONSUMPTION_RULE}${CHILD_FUTURE_STORY_RULE}${PV_ONLY_BATTERY_TEASE_RULE}${PLAN_REVIEW_RULE}
